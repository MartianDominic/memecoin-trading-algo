...
exports.RugCheckService = void 0;
...
class RugCheckService {
    ...
    async analyzeToken(tokenAddress, filters) {
        try {
            ...
            const analysis = this.performSecurityAnalysis(tokenAddress, result.metadata, result.accounts, result.liquidity);
            // Apply filters
            if (filters) {
            ...
            cache_1.globalCache.set(cacheKey, analysis, 300);
    ...
    performSecurityAnalysis(address, metadata, accounts, liquidity) {
        const risks = [];
        const warnings = [];
    ...
    calculateHolderConcentration(accounts) {
    ...
    checkSuspiciousName(name, symbol) {
        const checkText = `${name || ''} ${symbol || ''}`.toLowerCase();
        return this.suspiciousPatterns.some(pattern => checkText.includes(pattern.toLowerCase()));
    }
    detectHoneypotRisk(metadata, accounts) {
    ...
    applySecurityFilters(analysis, filters) {
        // Safety score filter
    ...
    simulateLiquidityAnalysis(address) {
        return {
    ...
    async storeInHiveMemory(key, data) {
        try {
            cache_1.globalCache.set(`hive:rugcheck:${key}`, data, 300);
        }
        catch (error) {
...
exports.RugCheckService = RugCheckService;
