...
exports.TokenPipelineService = void 0;
const events_1 = require("events");
const logger_1 = require("../utils/logger");
const cache_1 = require("../utils/cache");
class TokenPipelineService extends events_1.EventEmitter {
    constructor(dexScreenerService, rugCheckService, jupiterService, solscanService, config = {
        ...
        this.maxProcessingTimeHistory = 1000;
        this.setupEventListeners();
    }
    setupEventListeners() {
        this.on('stage:start', (stage, tokenAddress) => {
            this.logger.debug(`Pipeline stage started: ${stage}`, { stage, tokenAddress });
        });
        this.on('stage:complete', (stage, tokenAddress, result) => {
        ...
        this.on('token:complete', (analysis) => {
            this.updateStats(analysis);
        });
        this.on('pipeline:error', (error, context) => {
     ...
     */
    async processToken(tokenAddress, filters) {
        ...
        try {
            ...
            const analysis = this.createCombinedAnalysis(tokenAddress, dexResult.data, rugResult.data, jupiterResult.data, solscanResult.data);
            ...
            const processingTime = Date.now() - startTime;
            this.recordProcessingTime(processingTime);
            this.emit('pipeline:error', error, { tokenAddress, processingTime });
            return this.createFailedAnalysis(tokenAddress, 'pipeline', error instanceof Error ? error.message : 'Unknown pipeline error');
        }
    }
    /**
     * Process multiple tokens in parallel batches
     */
    async processBatch(tokenAddresses, filters) {
        ...
        for (let i = 0; i < tokenAddresses.length; i += this.config.batchSize) {
            ...
            const batchPromises = batch.map(address => this.processTokenWithTimeout(address, filters));
    ...
    async processTokenWithTimeout(tokenAddress, filters) {
        return Promise.race([
            this.processToken(tokenAddress, filters),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Pipeline timeout')), this.config.timeoutMs))
        ]).catch(error => {
    ...
    async processConcurrent(promises, maxConcurrent) {
        ...
        for (let i = 0; i < promises.length; i += maxConcurrent) {
            ...
            results.push(...chunkResults.map(result => result.status === 'fulfilled' ? result.value : null).filter(result => result !== null));
        }
        return results;
    }
    async processDexScreenerStage(tokenAddress, filters) {
    ...
    async processRugCheckStage(tokenAddress, filters) {
    ...
    async processJupiterStage(tokenAddress, filters) {
        const stage = 'jupiter';
        const startTime = Date.now();
        this.emit('stage:start', stage, tokenAddress);
        try {
    ...
    async processSolscanStage(tokenAddress, filters) {
        const stage = 'solscan';
        const startTime = Date.now();
        this.emit('stage:start', stage, tokenAddress);
        try {
    ...
    createCombinedAnalysis(tokenAddress, dexScreener, rugCheck, jupiter, solscan) {
        ...
        const failedFilters = [];
        if (dexScreener.filterReason)
            failedFilters.push(`DEX: ${dexScreener.filterReason}`);
        if (rugCheck.filterReason)
            failedFilters.push(`Security: ${rugCheck.filterReason}`);
        if (jupiter.filterReason)
            failedFilters.push(`Routing: ${jupiter.filterReason}`);
        if (solscan.filterReason)
            failedFilters.push(`Creator: ${solscan.filterReason}`);
        return {
    ...
    calculateOverallScore(dexScreener, rugCheck, jupiter, solscan) {
        let score = 0;
        let weights = 0;
        // DEXScreener contribution (25% weight)
        const dexWeight = 25;
        let dexScore = 50; // Base score
        if (dexScreener.liquidity > 10000)
            dexScore += 20;
        if (dexScreener.volume24h > 5000)
            dexScore += 15;
        if (dexScreener.age < 24 && dexScreener.age > 1)
            dexScore += 15;
        ...
        if (jupiter.slippageEstimate < 5)
            jupiterScore += 25;
        else if (jupiter.slippageEstimate < 10)
            jupiterScore += 15;
        if (!jupiter.blacklisted)
            jupiterScore += 15;
        ...
        if (solscan.creatorInfo.ruggedTokens === 0)
            solscanScore += 25;
        else if (solscan.creatorInfo.ruggedTokens <= 1)
            solscanScore += 10;
        if (solscan.topHoldersPercentage < 40)
            solscanScore += 15;
        else if (solscan.topHoldersPercentage < 60)
            solscanScore += 5;
        if (solscan.fundingPattern === 'organic')
            solscanScore += 10;
        score += (solscanScore * solscanWeight) / 100;
        weights += solscanWeight;
        return Math.min(100, Math.max(0, score));
    }
    createFailedAnalysis(tokenAddress, failedStage, reason) {
    ...
    getCachedAnalysis(tokenAddress) {
        return cache_1.globalCache.get(`pipeline:analysis:${tokenAddress.toLowerCase()}`);
    }
    cacheAnalysis(tokenAddress, analysis) {
        cache_1.globalCache.set(`pipeline:analysis:${tokenAddress.toLowerCase()}`, analysis, 600); // 10 minutes
    }
...
exports.TokenPipelineService = TokenPipelineService;
//# sourceMappingURL=token-pipeline.service.js.map