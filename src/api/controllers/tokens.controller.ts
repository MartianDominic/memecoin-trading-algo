// Tokens Controller - Token CRUD and Search Endpoints
...
export class TokensController {
  ...
  public async listTokens(req: Request, res: Response): Promise<void> {
    try {
      // Validate query parameters
      const query = tokenListQuerySchema.parse(req.query);
      ...
      if (query.minMarketCap || query.maxMarketCap) {
        ...
        if (query.minMarketCap) whereClause.marketCap.gte = query.minMarketCap;
        if (query.maxMarketCap) whereClause.marketCap.lte = query.maxMarketCap;
      }
  ...
  public async createOrUpdateToken(req: Request, res: Response): Promise<void> {
    try {
  ...
  private calculatePerformanceMetrics(priceData: any[]): any {
  ...
  private calculateROI(prices: any[], periodMs: number): number | undefined {
    ...
    const historicalPrice = prices.find(p =>
      new Date(p.timestamp).getTime() >= periodStart
    )?.price;

    if (!currentPrice || !historicalPrice) return undefined;
