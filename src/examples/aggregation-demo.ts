...
import { TokenFilterCriteria } from '../types/api.types';
...
async function demonstrateTokenAggregation() {
  ...
  const aggregator = new TokenAggregatorService(
    dexScreenerService,
    rugCheckService,
    jupiterService,
    solscanService,
  ...
  await demonstratePipelineProcessing(
    dexScreenerService,
    rugCheckService,
    jupiterService,
    solscanService,
 ...
 */
function setupEventListeners(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
 ...
 */
async function demonstrateManualRun(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
 ...
 */
async function demonstrateScheduledAggregation(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
 ...
 */
async function demonstratePipelineProcessing(
  dexScreenerService: DexScreenerService,
  rugCheckService: RugCheckService,
  jupiterService: JupiterService,
  solscanService: SolscanService,
  ...
  const pipeline = new TokenPipelineService(
    dexScreenerService,
    rugCheckService,
    jupiterService,
    solscanService
 ...
 */
async function demonstrateHealthMonitoring(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
 ...
 */
async function demonstrateConfigurationManagement(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
 ...
 */
async function demonstrateErrorHandling(aggregator: TokenAggregatorService) {
  const logger = Logger.getInstance();
...
// Export the demo function for easy execution
export { demonstrateTokenAggregation };
